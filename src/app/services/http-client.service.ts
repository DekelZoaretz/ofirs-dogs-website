...
const defaultTransformResponse =
  (errorCode?: keyof typeof eHttpStatusCode) =>
  (data: any): HttpResponseType<any> => {
    return {
...
const handleResponse = async (
  request: Promise<any>,
): Promise<any | { data: any; errorCode: string }> => {
  try {
...
export const HttpClient = {
  ...
  delete<T = any, R = T>(url: string, config?: AxiosRequestConfig): Promise<R> {
    return handleResponse(
      axiosInstance.delete<T, R>(url, getFullConfig(config)),
    );
  },
  post<T = any, R = T>(
    url: string,
    ...
    return handleResponse(
      axiosInstance.post<T, R>(url, payload, getFullConfig(config)),
    );
  },
  put<T = any, R = T>(
    url: string,
    ...
    return handleResponse(
      axiosInstance.put<T, R>(url, payload, getFullConfig(config)),
    );
  },
  patch<T = any, R = T>(
    url: string,
    ...
    return handleResponse(
      axiosInstance.patch<T, R>(url, payload, getFullConfig(config)),
    );
  },
};
